//
//  exportTripData.swift
//  ViDrive
//
//  Created by David Holeman on 4/14/24.
...
func exportTripData(tripTimestamp: Date) -> Int {
    LogEvent.print(module: "exportTripData()", message: "Exporting trip data ...")
    ...
    do {
        ...
        for item in sortedTrip {
            let tripItem = TripItemJSON(timestamp: item.timestamp, latitude: item.latitude, longitude: item.longitude, speed: item.speed, processed: false, code: item.code, note: item.note)
        ...
        do {
            ...
            print("** Trip items were successfully saved to \(String(describing: fileURL))")
        } catch {
        ...
        do {
            ...
            LogEvent.print(module: "exportTripSData()", message: "An error occurred saving the export file: \(error)")

        }

    } catch {
        LogEvent.print(module: "exportTripSData()", message: "An error occurred: \(error)")
    }
